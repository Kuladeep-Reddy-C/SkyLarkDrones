/**
 * A compact, always-fresh schema + data snapshot summary that is injected
 * directly into the system prompt. This lets the agent answer most questions
 * WITHOUT first spending an LLM round-trip on `get_data_overview` — it already
 * knows the field names, the key enum values and the current caveats.
 */
import { getSnapshot } from '../data/store.js';

function uniq(rows, field, n = 7) {
  const s = new Set();
  for (const r of rows) {
    const v = r[field];
...
export async function buildSchemaBrief() {
