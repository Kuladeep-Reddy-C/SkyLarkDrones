...
import { DEALS_BOARD, WORK_ORDERS_BOARD } from './schema.js';
import {
  cleanText, parseDate, parseNumber, parseQuantity,
  normalizeSector, normalizeDealStatus, parseDealStage, normalizeProbability,
} from './normalize.js';
import { analyseQuality } from './quality.js';

let cache = null; // { snapshot, fetchedAt }
let inflight = null;
...
async function resolveBoardIds() {
  ...
  if (!dealsId || !woId) {
    ...
    const byName = (n) => boards.find((b) => b.name.trim().toLowerCase() === n.trim().toLowerCase());
...
function toRawMap(item) {
  const map = { Name: item.name };
...
function normalizeDeal(item) {
  ...
  if (prob.pct === null && status.status === 'Open') issues.push('open deal without closure probability');
...
async function buildSnapshot() {
  ...
  const [dealItems, woItems] = await Promise.all([
