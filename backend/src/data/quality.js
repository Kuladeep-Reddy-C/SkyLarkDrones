...
function fieldFill(rows, field) {
  const filled = rows.filter((r) => {
    const v = r[field];
    if (v && typeof v === 'object' && 'value' in v) return v.value !== null;
    return v !== null && v !== undefined && v !== '';
  }).length;
  return { field, filled, missing: rows.length - filled, missingPct: pct(rows.length - filled, rows.length) };
}

export function analyseQuality(deals, workOrders) {
  const dealFields = ['dealValue', 'sector', 'dealStatus', 'dealStage', 'closureProbability', 'createdDate', 'ownerCode', 'clientCode'];
  const woFields = ['amountInGst', 'sector', 'executionStatus', 'poDate', 'billedInGst', 'collectedInGst', 'invoiceStatus', 'collectionStatus', 'billingStatus'];

  const dealIssueCounts = {};
  for (const d of deals) for (const i of d._issues) dealIssueCounts[i] = (dealIssueCounts[i] || 0) + 1;
  const woIssueCounts = {};
  for (const w of workOrders) for (const i of w._issues) woIssueCounts[i] = (woIssueCounts[i] || 0) + 1;
  ...
  if (dealNoValue) {
    notes.push(`${dealNoValue}/${deals.length} deals have no deal value and are excluded from revenue/pipeline sums.`);
  }
  const sectorMismatch = new Set([
  ...
  return {
    deals: {
      total: deals.length,
      fill: dealFields.map((f) => fieldFill(deals, f)),
      issues: dealIssueCounts,
    },
    workOrders: {
      total: workOrders.length,
      fill: woFields.map((f) => fieldFill(workOrders, f)),
    ...
    crossBoard: {
      ...
      knownSectors: [...sectorMismatch].sort(),
    },
    notes,
  };
}
