...
const NULLISH = new Set([
  '', '-', '--', 'n/a', 'na', 'none', 'null', 'nil', 'tbd', 'not applicable',
  'not available', '#n/a', '#value!', 'unknown',
]);
...
/** Title-case a normalised label while keeping known acronyms upper. */
export function titleCase(v) {
  ...
  return s
    .toLowerCase()
    .split(' ')
    .map((w) => (w.length <= 3 && /^[a-z]+$/.test(w) && ['and', 'the', 'for', 'of'].includes(w)
      ? w
...
/**
 * Parse a messy date into { iso: 'YYYY-MM-DD'|null, monthKey: 'YYYY-MM'|null, raw }.
 * Returns null only when the input is truly empty.
 */
export function parseDate(v) {
  ...
  // Native Date object (from xlsx cellDates)
  if (v instanceof Date && !Number.isNaN(v.getTime())) {
    return toParts(v, raw);
  }
  ...
  // Bare month name -> assume current or upcoming fiscal context; keep monthKey only
  ...
  if (mY && MONTHS[mY[1].toLowerCase()] !== undefined) {
    ...
    return { ...toParts(d, raw), iso: null }; // month precision only
  }
  ...
  if (dmy) {
    ...
    if (y < 100) y += 2000;
    if (m > 12 && d <= 12) [d, m] = [m, d]; // swap if clearly month-first
...
export function parseNumber(v) {
  ...
  s = s.replace(/[₹$,]/g, '').replace(/\s*(ha|acre|acres|km|sq\.?\s?km|units?|nos?\.?|kms)\b/gi, '').trim();
  s = s.replace(/,/g, '');
  const m = s.match(/-?\d+(\.\d+)?/);
  if (!m) return null;
  const n = Number(m[0]);
...
export function parseQuantity(v) {
  ...
  return { value: parseNumber(v), raw: text };
}
...
export function normalizeSector(v) {
  const s = cleanText(v);
...
export function normalizeDealStatus(v) {
  const s = cleanText(v);
...
export function parseDealStage(v) {
  const s = cleanText(v);
  ...
  if (m) {
    ...
    return {
      ...
      code,
      name: titleCase(m[2]),
      rank: code.charCodeAt(0) - 64, // A=1
    };
  }
  return { stage: s, code: null, name: titleCase(s), rank: null };
}

const PROBABILITY_PCT = { high: 0.8, medium: 0.5, med: 0.5, low: 0.2 };

export function normalizeProbability(v) {
  const s = cleanText(v);
  if (!s) return { label: null, pct: null };
  const key = s.toLowerCase();
  if (PROBABILITY_PCT[key] !== undefined) {
    return { label: titleCase(s), pct: PROBABILITY_PCT[key] };
  }
...
export function looksLikeHeaderRow(record, headerTokens) {
