...
export default function ReportDrawer({ onClose }) {
    ...
    <>
      ...
      <div className="drawer" role="dialog" aria-label="Leadership briefing">
        <div className="drawer-head">
          ...
          <button className="icon-btn" onClick={onClose} aria-label="Close"><Icon.close width={15} height={15} /></button>
        </div>
          ...
          {state.loading && (
            <>
              ...
              {Array.from({ length: 9 }).map((_, i) => (
                <div key={i} className="skeleton-line" style={{ width: `${60 + ((i * 37) % 40)}%` }} />
              ))}
            </>
          )}
