...
const PROMPTS: { icon: IconName; cat: string; q: string }[] = [
  {
    icon: 'spark',
    cat: 'Pipeline',
  ...
  },
  {
    icon: 'coin',
    cat: 'Revenue',
    q: 'Total open pipeline value, weighted by probability, broken down by stage',
  },
  { icon: 'layers', cat: 'Sectors', q: 'Which sectors generate the most delivered revenue?' },
  {
    icon: 'wallet',
    cat: 'Collections',
    q: 'How are collections? Any large receivables outstanding?',
  },
...
export default function EmptyState({ onPick, counts }: Props) {
  return (
    <div className="empty">
      ...
      <h2 className="greeting">
        {greeting()} <em>What would you like to know?</em>
      </h2>
      <p className="sub">
        Ask a founder-level question about the sales pipeline or project execution. I read the live
        Deals and Work Orders boards from Monday.com
