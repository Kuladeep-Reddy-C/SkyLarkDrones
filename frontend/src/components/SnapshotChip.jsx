...
export default function SnapshotChip({ overview, onRefresh }) {
  ...
  return (
    <>
      <span className="snapshot-chip">
        ...
        <button onClick={refresh} title="Refresh from Monday.com" style={{ display: 'inline-flex' }}>
          <Icon.refresh width={12} height={12} style={{ animation: spinning ? 'spin .7s linear infinite' : 'none' }} />
        </button>
        {caveats.length > 0 && (
      ...
      {open && (
        <div className="caveat-pop">
          <h4 className="eyebrow" style={{ color: 'var(--warn)' }}>Data-quality caveats</h4>
          <ul>
